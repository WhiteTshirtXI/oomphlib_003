...
.
 ...
 in private containers (e.g. vectors); they are usally accompanied 
 ...
 - Use a prefix `\c n' for the access function for the 
   ...
   unsigned nelement();
   \endcode
 \b Notes: (i) No underscore betweeen the "n" and the container's name.
...
- Use const whereever applicable (arguments, member functions,...)
...
- Don't use macros! There is a single exception to this rule:
  We use the macro  \c OOMPH_EXCEPTION_LOCATION to make
  the file name and line number available to the
  \c OomphLibException object -- the object that is
  thrown if a run-time error is detected.
