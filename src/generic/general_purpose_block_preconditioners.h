...
namespace oomph
{
 ...
 preconditioner_solve(const DoubleVector& r, DoubleVector& z)
 {
  ...
  if(this->Prec_blocks_has_been_set)
  {
    n_block = this->Block_to_block_map.size();
  }
  else
  {
  ...
  for (unsigned i = 0; i < nblock_types; i++)
   {
    ...
    Diagonal_block_preconditioner_pt[i]->setup(this->problem_pt(),block_matrix_pt);
 ...
 void ExactBlockPreconditioner<MATRIX>::setup()
 {
  ...
  Preconditioner_pt->setup(this->problem_pt(),exact_block_matrix_pt);
